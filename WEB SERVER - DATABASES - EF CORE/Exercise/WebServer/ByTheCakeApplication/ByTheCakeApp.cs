        ...
        private static void ConfigureRoutes(IAppRouteConfig appRouteConfig)
        {
            ...
            appRouteConfig
                .Post(
                    ...
                    (req.FormData["name"], req.FormData["price"], req.FormData["imageUrl"]));
            ...
            appRouteConfig
                .Get(
                    "/shopping/add/{(?<id>[0-9]+)}",
                    req => new ShoppingController().Order(req));
            ...
            appRouteConfig
                .Get(
                "/orders", req => new ShoppingController().ListOrders(req));

            appRouteConfig
               .Get
               ("/orderDetails/{(?<id>[0-9]+)}",
               req => new ShoppingController().Details(req));

            appRouteConfig
                .Get
                ("/orderDetails/cakeDetails/{(?<id>[0-9]+)}",
                req => new CakesController().Details(req));
        }
    }
}